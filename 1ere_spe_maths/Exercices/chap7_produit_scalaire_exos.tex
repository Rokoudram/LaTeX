...
\begin{enumerate}
    \item On considère deux vecteurs $\vv{u}$ et $\vv{v}$ tels que $\norme{\vv{u}}=6, \norme{\vv{v}}=4$ et $(\vv{u};\vv{v})=\frac{2\pi}{3}$\\
    Calculer $\scal{u}{v}$
    
    \item On considère trois points $A$, $B$ et $C$ tels que $AB=3$, $AC=4$ et $\vv{AB}\cdot\vv{AC}=5$\\
...
On considère les points $E$ et $F$ tels que $\vv{AE}=\vv{BC}+\frac{1}{2}\vv{BA}$ et $\vv{BF}=\frac{1}{2}\vv{AC}$
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item On considère trois points $D$, $E$ et $F$ tels que $DE=4$, $EF=6$ et $\vv{DE}\cdot\vv{EF}=8$\\
    Calculer la longueur $DF$
    \item On considère trois points $G$, $H$ et $I$ tels que $GH=3$, $GI=4$ et $\vv{GH}\cdot\vv{GI}=5$\\
