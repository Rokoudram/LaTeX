...
\begin{minipage}{0.4\linewidth}\vspace{-4cm}\hspace{-4cm}
	\begin{tikzpicture}[scale=1]
		\begin{axis}[
			xmin = -3, xmax = 3, ymin = 0, ymax = 5, xtick distance=1, ytick distance=1,
			...
			\addplot [domain=-3:3, samples=100, color=newred,line width=2pt]
			{e^(-x)};

			\addplot[domain = -3:3,color=newblue,samples=100,line width=2pt]
			{e^(2*x)};

			\addplot[domain = -3:3,color=newgreen,samples=100,line width=2pt]
			{2*e^x};

			\addplot[domain = -3:3,color=neworange,samples=100,line width=2pt]
			{e^(-0.5*x)};
		\end{axis}
	\end{tikzpicture}
\end{minipage}
