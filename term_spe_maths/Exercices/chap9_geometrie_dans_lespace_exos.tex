...
a) $(d_1):\begin{cases}x=3+2t\\y=2-t\\z=-2t\end{cases}$ \qquad
$(d_2):\begin{cases}x=-3-4t\\y=1+2t\\z=-9+4t\end{cases}$ \\[15pt]
b) $(d_1):\begin{cases}x=1-t\\y=3+2t\\z=2-3t\end{cases}$ \qquad
$(d_2):\begin{cases}x=3t+6\\y=7+t\\z=2t+3\end{cases}$ \\[15pt]
c) $(d_1):\begin{cases}x=5t+6\\y=7-2t\\z=t+3\end{cases}$ \qquad 
$(d_2):\begin{cases}x=1+4t\\y=3-3t\\z=8t-2\end{cases}$ 



\subsection{Trois droites}
On considère les droites $(d_1):\begin{cases}x=3+t\\y=-3+2t\\z=10+3t\end{cases}$,\quad$(d_2):\begin{cases}x=2-2t\\y=-5\\z=7+t\end{cases}$ et $(d_3):\begin{cases}x=5+3t\\y=-4+t\\z=2-5t\end{cases}$
\begin{enumerate}
...
On considère la droite $(\Delta_1)$ passant par l'origine du repère et dirigée par le vecteur $\vcoord{v}{1\\k\\2}$ pour $k\in\RR$ ainsi que la droite $(\Delta_2): \begin{cases}x=t-2\\y=t+3\\z=t\end{cases}$
\begin{enumerate}
...
On considère les droites $(d):\begin{cases}x=3t+2\\y=3t-1\\z=-t+1\end{cases}$ et $(d'):\begin{cases}x=2t-2\\y=t+1\\z=-t+2\end{cases}$
\begin{enumerate}
...
\begin{enumerate}
	\item La droite $(d):\begin{cases}x=t-1\\y=2+3t\\z=2\end{cases}$ et le plan $\Pcal : 2x+y-3z-1=0$
	\item La droite $(AB)$ avec $A(2;-5;3)$ et $B(1;1;1)$ ainsi que le plan $\Rcal :x-y=5$
	\item Le plan $(PQR)$ avec $P(0;-3;2)$, $Q(1;-1;-1)$ et $R(3;-2;4)$ ainsi que la droite $\Delta :\begin{cases}x=2+t\\y=1-2t\\z=3t\end{cases}$
...
$\Pcal : \begin{cases}x=1+t+s\\y=2-t+s\\z=3+2t-s\end{cases}$ $t,s\in\RR$ \hspace{2cm} et \hspace{2cm} $\Rcal : \begin{cases}x=2-t-2s\\y=t+2s\\z=1+t-s\end{cases}$ $t,s\in\RR$
\begin{enumerate}
